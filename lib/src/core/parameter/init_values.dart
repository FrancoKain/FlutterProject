...
class InitCore {
  ...
  Future<bool> initialize() async {
    _floorDatabase = await $FloorFloorMovieDatabase.databaseBuilder('movie_database_v2').build();
    _movieApiService = ApiMovieService();
    _movieDatabase = MovieDatabase(_floorDatabase);
    _moviesRepositoryFromApiService =
        MoviesRepositoryFromApiService(api: _movieApiService, movieDatabase: _movieDatabase);
    _moviesRepositoryFromDataBase =
        MoviesRepositoryFromDataBase(api: _movieApiService, movieDatabase: _movieDatabase);


    _genreRepositoryFromApi = GenreRepositoryFromApi(api: GenreApiProvider(), movieDatabase: _movieDatabase);
    _genreRepositoryFromDatabase = GenreRepositoryFromDatabase(movieDatabase: _movieDatabase);
    ...
    _getGenresUseCase = GetGenresUseCase(genreRepository: _genreRepositoryFromApi, genreRepositoryFromDatabase: _genreRepositoryFromDatabase);
